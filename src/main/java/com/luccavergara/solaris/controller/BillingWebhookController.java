...
import com.luccavergara.solaris.billing.MercadoPagoWebhookValidator;
...
@RestController
@RequestMapping("/api/v1/billing/mercadopago")
...
public class BillingWebhookController {
    ...
    public ResponseEntity<Void> handleWebhook(
        ...
        if (!mercadoPagoWebhookValidator.isValid(signature, requestId, paymentId)) {
            log.warn("Rejected Mercado Pago webhook with invalid signature for payment {}", paymentId);
            return ResponseEntity.status(401).build();
        }

        try {
            mercadoPagoBillingService.processPaymentNotification(paymentId);
        } catch (Exception ex) {
    ...
    public ResponseEntity<Void> handleWebhookGet(
        ...
        try {
            mercadoPagoBillingService.processPaymentNotification(id);
        } catch (Exception ex) {
    ...
    private String extractNested(Map<String, Object> body, String key) {
        ...
        return body.get(key) != null ? String.valueOf(body.get(key)) : null;
    }
